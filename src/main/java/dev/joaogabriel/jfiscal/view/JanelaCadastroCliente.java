...
public class JanelaCadastroCliente {
    ...
    public void initialize() {
        ...
        mostrarMensagem("Insira os dados do cliente a ser cadastrado.", false);
    }
    ...
    public void cadastrarEmitente(ActionEvent actionEvent) {
        mostrarMensagem("Cadastrando dados do cliente no servidor. Por favor, aguarde...", true);
        ...
        Call<Empresa> call = FiscalConfig.getInstance().create(FiscalWS.class).cadastrarCliente(empresa);
        call.enqueue(new Callback<Empresa>() {
            @Override
            public void onResponse(@NotNull Call<Empresa> call, @NotNull Response<Empresa> response) {
                if (response.isSuccessful()) {
                    Platform.runLater(() -> mostrarMensagem("Cliente cadastrado com sucesso!.", false));
                } else Platform.runLater(() -> mostrarMensagem("Não foi possível concluir o cadastro do cliente.", false));
            }
            @Override
